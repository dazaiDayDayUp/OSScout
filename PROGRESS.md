...
| 事项 | 说明 |
|------|------|
| 邮件推送功能 | FastAPI-Mail + Celery 异步任务，分析完成后自动发送 HTML 邮件报告（含 SVG 评分环、CSS 条形图、关键发现）；前端首页新增可选邮箱输入框 |
| 前端 tsconfig 修复 | `tsconfig.json` 改为 TypeScript 5.0+ 解决方案文件格式，消除 `"baseUrl"` 报红 |
| 代码清理 | 删除临时脚本 `generate_self_rag_demo.py`；移除未使用导入；修复 `.gitignore` 保护 `backend/.env` |
